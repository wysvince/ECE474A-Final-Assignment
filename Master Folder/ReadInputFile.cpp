...
int ReadInputFile::handleStatements(ifstream &file, Graph* graph) {
	string inputLine;
	string cond;
	string ifElse;
	Edges tempEdge;
	...
	while (!file.eof()) {
		getline(file, inputLine);
		istringstream inputLineStream(inputLine);
		inputLineStream >> ifElse;

		if (ifElse.compare("if") == true) {
			inputLineStream >> cond >> cond >> cond >> cond; // reads " ( cond", where cond is our condition
			saveCond = cond;

			tempEdge.setCondtionalOperation(cond);
			ifOpen = true;
		}
		else if (ifElse.compare("else") == true) {
			cond = ifElse;	// It's the else portion so label it.

			tempEdge.setCondtionalOperation(cond);
			ifOpen = true;

		}
		else if (ifElse.compare("input") == true) {			// for variables.
			// do nothing.
		}
		else if (ifElse.compare("output") == true) {		// for variables.
			// do nothing.
		}
		else if (ifElse.compare("variable") == true) {		// for variables.
			// do nothing
		}
		else if (ifElse.compare(" ") == true) {				// for empty line.
			// do nothing
		}
		else if (ifElse.compare("}") == true) {				// for end of if-statement
